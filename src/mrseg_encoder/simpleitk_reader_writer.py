...
class SimpleITKIO:
    ...
    def transform_image(
        ...
        if force_spacing:
            if verbose: print("Forcing spacing")
            x,y,z = itk_image.GetSize()
            if verbose: print(f"Image size: {x}, {y}, {z}")
            # spacing = ( 1.0 * 64/ x, 1.0*64 / y, 1.0*16 / z)  # force spacing to 1.5mm isotropic
            spacing = ( 1.0 * 64/ x, 1.0*64 / y, 1.0*64 / z)  # force spacing to 1.5mm isotropic
            if verbose: print(f"New spacing: {spacing}")
        else:
        ...
        assert (
            npy_image.ndim == 3
        ), f"Unexpected number of dimensions: {npy_image.ndim}"
        ...
        return npy_image.astype(np.float32), _dict, itk_image
