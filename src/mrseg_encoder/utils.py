...
def read_images(namespace: Any) -> List[str]:
    ...
    condition: Callable[[str], bool] = (
        lambda x: x[-7:] == ".nii.gz" or x[-4:] == ".nii" or x[-4:] == ".mha"
    )
    ...
    if os.path.isdir(namespace.input):
        ...
        assert condition(
            images[0]
        ), f"file ending of {namespace.input} neither .nii, .nii.gz, nor .mha"
...
def add_postfix(name: str, postfix: str) -> str:
    ...
    elif Path(name).suffix == ".mha":
        return Path(name).stem + "_" + postfix + ".mha"
    else:
        raise ValueError("Files must end with either .nii, .nii.gz. or .mha")
